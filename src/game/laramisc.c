...
void LaraControl(int16_t item_num)
{
    ...
    if (Lara.water_status == LWS_ABOVEWATER && room_submerged) {
        ...
        if (item->current_anim_state == AS_SWANDIVE) {
            ...
            LaraFallSpeedF *= 1.5 / (double)ANIM_SCALE;
        }
        Lara.head_x_rot = 0;
        Lara.head_y_rot = 0;
        Lara.torso_x_rot = 0;
...
void AnimateLara(ITEM_INFO *item)
{
        ...
        if (anim->number_commands > 0) {
            ...
            for (int i = 0; i < anim->number_commands; i++) {
                ...
                case AC_JUMP_VELOCITY: {
                    ...
                    LaraSpeedF = (double)dummy / (double)ANIM_SCALE;
    ...
    } else // ANIM_SCALE = 2
    {
        if (item->gravity_status) {
            ...
            LaraFloatPos.y += LaraFallSpeedF / (double)ANIM_SCALE;
