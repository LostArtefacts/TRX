...
void DrawEffect(int16_t fxnum)
{
    ...
    if (object->nmeshes < 0) {
        S_DrawSprite(
            ...
            object->mesh_index - fx->frame_number / AnimScale, 4096);
    } else {
        phd_PushMatrix();
        ...
        if (PhdMatrixPtr->_23 > PhdNearZ && PhdMatrixPtr->_23 < PhdFarZ) {
            ...
            if (object->nmeshes) {
                ...
                phd_PutPolygons(Meshes[fx->frame_number / AnimScale], -1);
            }
        }
        phd_PopMatrix();
    }
...
void DrawLara(ITEM_INFO *item)
{
    ...
    *PhdMatrixPtr = saved_matrix;
    ...
    case LGT_UZIS:
        ...
        packed_rotation =
            (int32_t
                 *)(Lara.right_arm.frame_base + (Lara.right_arm.frame_number / AnimScale) * (object->nmeshes * 2 + FRAME_ROT) + 10);
        phd_RotYXZ(
        ...
        packed_rotation =
            (int32_t
                 *)(Lara.left_arm.frame_base + (Lara.left_arm.frame_number / AnimScale) * (object->nmeshes * 2 + FRAME_ROT) + 10);
        phd_RotYXZ(
