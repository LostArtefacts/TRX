...
void DrawSpriteItem(ITEM_INFO *item)
{
    S_DrawSprite(
        ...
        Objects[item->object_number].mesh_index - (item->frame_number/ANIM_SCALE),
        item->shade);
}
...
void DrawLara(ITEM_INFO *item)
{
    ...
    *PhdMatrixPtr = saved_matrix;
    ...
    case LGT_UZIS:
        ...
        packed_rotation =
            (int32_t
                 *)(Lara.right_arm.frame_base + (Lara.right_arm.frame_number / ANIM_SCALE ) * (object->nmeshes * 2 + FRAME_ROT) + 10);
        phd_RotYXZ(
        ...
        packed_rotation =
            (int32_t
                 *)(Lara.left_arm.frame_base + (Lara.left_arm.frame_number / ANIM_SCALE ) * (object->nmeshes * 2 + FRAME_ROT) + 10);
        phd_RotYXZ(
