...
void Gun_Pistols_Draw(LARA_GUN_TYPE weapon_type)
{
    ...
    if (!Item_TestAbsFrameRange(ani, LF_G_DRAW1, LF_G_DRAW2_L)) {
        ani = LF_G_DRAW1;
    } else if (Item_TestAbsFrameEqual(ani, LF_G_DRAW2)) {
        Gun_Pistols_DrawMeshes(weapon_type);
        Sound_Effect(SFX_LARA_DRAW, &g_LaraItem->pos, SPM_NORMAL);
    } else if (Item_TestAbsFrameEqual(ani, LF_G_DRAW2_L)) {
        Gun_Pistols_Ready();
        ani = LF_G_AIM;
    }
...
void Gun_Pistols_Undraw(LARA_GUN_TYPE weapon_type)
{
    ...
    if (Item_TestAbsFrameRange(anil, LF_G_RECOIL, 1024)) {
        anil = LF_G_AIM_L;
    } else if (Item_TestAbsFrameRange(anil, LF_G_AIM + 1, LF_G_DRAW1 - 1)) {
        g_Lara.left_arm.x_rot -= g_Lara.left_arm.x_rot / anil;
        g_Lara.left_arm.y_rot -= g_Lara.left_arm.y_rot / anil;
        anil--;
    } else if (Item_TestAbsFrameEqual(anil, LF_G_AIM)) {
        g_Lara.left_arm.x_rot = 0;
        g_Lara.left_arm.y_rot = 0;
        g_Lara.left_arm.z_rot = 0;
        anil = LF_G_DRAW2_L;
    } else if (Item_TestAbsFrameRange(anil, LF_G_DRAW1 + 1, 1024)) {
        anil--;
        if (Item_TestAbsFrameEqual(anil, LF_G_DRAW2)) {
    ...
    if (anir >= LF_G_RECOIL) {
        anir = LF_G_AIM_L;
    } else if (Item_TestAbsFrameRange(anir, LF_G_AIM + 1, LF_G_DRAW1 - 1)) {
        g_Lara.right_arm.x_rot -= g_Lara.right_arm.x_rot / anir;
        g_Lara.right_arm.y_rot -= g_Lara.right_arm.y_rot / anir;
        anir--;
    } else if (Item_TestAbsFrameEqual(anir, LF_G_AIM)) {
        g_Lara.right_arm.x_rot = 0;
        g_Lara.right_arm.y_rot = 0;
        g_Lara.right_arm.z_rot = 0;
        anir = LF_G_DRAW2_L;
        // } else if (anir > LF_G_DRAW1) {
    } else if (Item_TestAbsFrameRange(anir, LF_G_DRAW1 + 1, 1024)) {
        anir--;
        if (Item_TestAbsFrameEqual(anir, LF_G_DRAW2)) {
            Gun_Pistols_UndrawMeshRight(weapon_type);
        }
    }
    g_Lara.right_arm.frame_number = anir;

    if (Item_TestAbsFrameEqual(anil, LF_G_DRAW1)
        && Item_TestAbsFrameEqual(anir, LF_G_DRAW1)) {
...
void Gun_Pistols_Animate(LARA_GUN_TYPE weapon_type)
{
    ...
    if (g_Lara.right_arm.lock || (g_Input.action && !g_Lara.target)) {
        if (Item_TestAbsFrameRange(anir, LF_G_AIM, LF_G_AIM_L - 1)) {
            anir++;
        } else if (Item_TestAbsFrameEqual(anir, LF_G_AIM_L) && g_Input.action) {
        ...
        anir = LF_G_AIM_L;
    } else if (Item_TestAbsFrameRange(anir, LF_G_AIM + 1, LF_G_AIM_L)) {
        anir--;
    }
    g_Lara.right_arm.frame_number = anir;
    ...
    if (g_Lara.left_arm.lock || (g_Input.action && !g_Lara.target)) {
        if (Item_TestAbsFrameRange(anil, LF_G_AIM, LF_G_AIM_L - 1)) {
            anil++;
        } else if (Item_TestAbsFrameEqual(anil, LF_G_AIM_L) && g_Input.action) {
        ...
        anil = LF_G_AIM_L;
    } else if (Item_TestAbsFrameRange(anil, LF_G_AIM + 1, LF_G_AIM_L)) {
        anil--;
    }
    g_Lara.left_arm.frame_number = anil;
}
