...
static const ENUM_TO_STRING map_GAME_STRING_ID[] = {
...
static const ENUM_TO_STRING map_GAMEFLOW_LEVEL_TYPE[] = {
    { "title", GFL_TITLE },
    { "normal", GFL_NORMAL },
    { "saved", GFL_SAVED },
    { "demo", GFL_DEMO },
...
static int32_t GameFlow_StringToEnumType(const char *str, const ENUM_TO_STRING *map)
{
...
{
    return (GAME_STRING_ID)GameFlow_StringToEnumType(str, map_GAME_STRING_ID);
}

static GAME_STRING_ID GameFlow_StringToLevelType(const char *str)
{
    return (GAMEFLOW_LEVEL_TYPE)GameFlow_StringToEnumType(str, map_GAMEFLOW_LEVEL_TYPE);
}
...
static bool GameFlow_LoadScriptLevels(struct json_object_s *obj)
{
    ...
    while (jlvl_elem) {
        ...
        cur->level_type = GameFlow_StringToLevelType(tmp_s);
