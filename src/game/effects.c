...
static int16_t m_NextFxActive = NO_ITEM;

void Effect_InitialiseArray(void)
{
    m_NextFxActive = NO_ITEM;
    m_NextFxFree = 0;
...
int16_t Effect_Create(int16_t room_num)
{
    ...
    fx->next_active = m_NextFxActive;
    m_NextFxActive = fx_num;
...
void Effect_Kill(int16_t fx_num)
{
    ...
    int16_t linknum = m_NextFxActive;
    if (linknum == fx_num) {
        m_NextFxActive = fx->next_active;
    } else {
