...
int32_t ExplodingDeath(int16_t item_num, int32_t mesh_bits, int16_t damage)
{
    ...
    if ((bit & mesh_bits) && (bit & item->mesh_bits)) {
        ...
        if (fx_num != NO_ITEM) {
            ...
            if (abortion) {
                fx->speed = (GetRandomControl() >> 7) / AnimScale;
                fx->fall_speed = (-GetRandomControl() >> 7) / AnimScale;
            } else {
                fx->speed = (GetRandomControl() >> 8) / AnimScale;
                fx->fall_speed = (-GetRandomControl() >> 8) / AnimScale;
            }
            fx->counter = damage;
...
void ControlBodyPart(int16_t fx_num)
{
    ...
    fx->fall_speed += GRAVITY / AnimScale;
