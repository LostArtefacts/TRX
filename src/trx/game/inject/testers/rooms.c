...
{
    const int32_t num_rooms = File_ReadS32(injection->fp);
...
static bool M_TestRoomMeta(
    const INJECTION_CONTEXT *const ctx, const INJECTION *const injection)
{
    const int32_t room_num = File_ReadS32(injection->fp);
    const int32_t x_pos = File_ReadS32(injection->fp);
    const int32_t z_pos = File_ReadS32(injection->fp);
    const int32_t min_floor = File_ReadS32(injection->fp);
    const int32_t max_ceiling = File_ReadS32(injection->fp);
    const uint16_t x_size = File_ReadU16(injection->fp);
    const uint16_t z_size = File_ReadU16(injection->fp);
