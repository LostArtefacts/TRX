...
bool Room_CheckOverlap(const int16_t room_num_0, const int16_t room_num_1)
{
    ...
    // clang-format off
    return (
        room_0_bounds.min.x + WALL_L < room_1_bounds.max.x - WALL_L &&
        room_0_bounds.min.y < room_1_bounds.max.y &&
        room_0_bounds.min.z + WALL_L < room_1_bounds.max.z - WALL_L &&
        room_0_bounds.max.x - WALL_L > room_1_bounds.min.x + WALL_L &&
        room_0_bounds.max.y > room_1_bounds.min.y &&
        room_0_bounds.max.z - WALL_L > room_1_bounds.min.z + WALL_L);
    // clang-format on
}
