...
void Lara_InitialiseInventory(const GF_LEVEL *const level)
{
    ...
    if (resume != nullptr) {
        ...
        if (resume->has_pistols) {
            Inv_AddItem(O_PISTOL_ITEM);
        }

        if (resume->has_magnums) {
        ...
        g_Lara.last_gun_type = resume->gun_type;
    }
...
void Lara_InitialiseMeshes(const GF_LEVEL *const level)
{
    ...
    if (resume->gun_type != LGT_UNARMED) {
        if (resume->gun_type == LGT_MAGNUMS) {
            holster_obj_id = O_LARA_MAGNUMS;
        } else if (resume->gun_type == LGT_UZIS) {
    ...
    } else if (resume->has_m16) {
        g_Lara.back_gun = O_LARA_M16;
    } else if (resume->has_grenade) {
        g_Lara.back_gun = O_LARA_GRENADE;
    } else if (resume->has_harpoon) {
        g_Lara.back_gun = O_LARA_HARPOON;
    }
}
